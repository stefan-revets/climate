...
A discrepancy has been noted between the CO_{2} measurements in
Greenland and Antarctic ice cores. \citet{anklin-al95:processes} set
out to try and explain why the Greenland values are much in excess,
compared to coeval Antarctic values. Part of the analysis deals with
analytical issues, i.e., the techniques used to liberate and measure
the CO_{2} content.
...
#+BEGIN_SRC R :file output/anklin_fig4.pdf
  ...
  anklin <- anklin_greenland %>%
      ...
      rename(Dry = Dry_ppm, Melt = Melt_ppm) %>%
...
#+END_SRC
Yes, there is not really a systematic offset when CO_{2} is determined
by the dry extraction technique. On the other hand, the melt-refreeze
technique shows a systematic deviation as well as a great deal of
spread. That is a disturbing result.

A more quantitative assessment of the variability, or spread of the
measurements, can be obtained get from the standard deviations
#+BEGIN_SRC R 
  anklin_greenland %>% select(-Age_AD) %>% summarise_all(funs(sd))
  anklin_antarctica %>% select(-Age_AD) %>% summarise_all(funs(sd))
#+END_SRC

#+RESULTS[e7bc6c6ffd330dc51aab0ff3df4aa64aaad11059]:
:    Dry_ppm Melt_ppm Delta_ppm
: 1 5.954383 26.08135  27.47782
:    Dry_ppm Melt_ppm Delta_ppm
: 1 2.452395 28.44366  28.33111
...
anything, the conclusion pushing itself forward is that the
melt-refreeze technique is not fit for purpose.
