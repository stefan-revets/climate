...
#+BEGIN_SRC R :results none
  ...
  h_planck <- 6.62606957e-34      # J s, Plank constant
...
#+BEGIN_SRC R :file output/solar_spectrum.pdf
  ...
  sun_energy <- data.frame(lambda = seq(100, 2500, 10)*1e-9) %>%
      mutate(E1 = 8 * pi * h_planck * c_light / lambda^5,
             E2 = h_planck * c_light / (lambda * k_boltzmann * T_sun),
             Energy = E1 / (exp(E2) - 1))
...
#+BEGIN_SRC awk :in-file data/hitran_co2.par :file cache/hitran_co2.dat
  BEGIN{OFS = ","
...
#+BEGIN_SRC awk :in-file data/hitran_h2o.par :file cache/hitran_h2o.dat
  BEGIN{OFS = ","
      FIELDWIDTHS = "2 1 12 10 10 5 5 10 4 8 60 6 12 1 7 7"
        print "nu, intensity, Einstein_A, gamma_air, gamma_self, \
               energy_lower, n_air, delta_air"}
...
It is a straightforward matter to import these csv files
...
Let's have a peek at the data for CO_{2}. As there are so many
entries, let's take a subsample for plotting
#+BEGIN_SRC R :file output/hitran_co2.pdf
  hitran_co2 %>%
      filter(nu > 500) %>%
      sample_n(2500, replace = TRUE) %>%
      ggplot() +
      geom_line(aes(1e4/nu, log(intensity))) +
      labs(title = "CO2 IR Absorption",
...
other molecules. In the lower atmosphere, the broadening of a line is
largely driven by pressure, and can be modeled as a Lorentz profile:
\begin{equation}
...
| \delta\isotope{11}{B} | \sigma |  n | technique |
|-----------------------+--------+----+-----------|
|                 37.26 |   0.22 |  2 | PTIMS     |
|                 38.84 |    0.4 |  6 | PTIMS     |
|                 39.42 |   0.03 |  3 | PTIMS     |
