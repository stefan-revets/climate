...
Daily CO_{2} measurements from 1971 onwards at Barrow, Alaska, Mauna
Loa, Hawaii, American Samoa, and Antarctica, provided by NOAA
\citep{thoning-al22:NOAA-CO2}
https://gml.noaa.gov/aftp/data/trace_gases/co2/in-situ/surface/
...
*** Rise of CO_{2}
NOAA has been collecting atmospheric CO_{2} concentrations for some 50
years \citep{thoning-al22:NOAA-CO2} at 4 stations (Mauna Loa, Barrow,
American Samoa and Antarctica).

#+BEGIN_SRC R :file output/noaa_co2.pdf
  noaa_barrow <- read.table('data/co2_brw_surface_daily.txt',
                            header = TRUE, skip = 149)
  noaa_maunaloa <- read.table('data/co2_mlo_surface_daily.txt',
                              header = TRUE, skip = 149)
  noaa_samoa <- read.table('data/co2_smo_surface_daily.txt',
                           header = TRUE, skip = 149)
  noaa_antarctica <- read.table('data/co2_spo_surface_daily.txt',
                                header = TRUE, skip = 149)

  noaa_co2 <- noaa_barrow %>%
      bind_rows(noaa_maunaloa) %>%
      bind_rows(noaa_samoa) %>%
      bind_rows(noaa_antarctica) %>%
      select(-hour, -minute, -second, -latitude, -longitude,
             -altitude, -elevation, -intake_height, -qcflag) %>%
      filter(value > 0)

  noaa_co2 %>%
      group_by(site_code, year, month) %>%
      summarise_at(vars(time_decimal, value), mean) %>%
      ggplot() +
      geom_line(aes(time_decimal, value, colour = site_code)) +
      labs(title = "NOAA CO2 Measurements",
           x = "Time",
           y = "CO2 (ppm)") +
      theme(legend.position = "bottom")
#+END_SRC

The plot reveals, apart from the dramatic and sustained rise in CO_{2}
concentration, a curious geographic differentiation. Concentrations
exhibit a consistent geographic trend, with the highest concentration
at the northernmost station (Barrow), followed by Mauna Loa, Samoa and
the lowest at the southernmost station (Antarctica). The summer-winter
differences follow the same trend, with largest differences in the
north and smallest in the south.

*** Ocean heat content
\citet{cheng-al23:heat} provided the latest update of the estimation
of the ocean's heat content in the upper \unit{2000}{\metre} of the
world's oceans up to 2022. From their figure 1, let's extract the
values calculated by the NCEI-NOAA teams (pentad annual means relative
to the 1981--2010 mean)

#+BEGIN_SRC R :file output/noaa_ohc.pdf
  noaa_ohc <- read.delim("data/ncei_noaa_ohc.dat", sep = ",")

  ggplot(noaa_ohc) +
      geom_bar(aes(year,ohc), stat = "identity") +
      labs(title = "NCEI-NOAA Ocean Heat Content",
           x = "Year",
           y = "Relative Heat Content (ZJ)")
#+END_SRC

From these values, let's see what the year on year changes are

#+BEGIN_SRC R :file output/noaa_ohc_change.pdf
  noaa_ohc %<>%
      mutate(ohc_change = ohc - lag(ohc, default = noaa_ohc$ohc[1]))

  ggplot(noaa_ohc) +
      geom_bar(aes(year, ohc_change), stat = "identity") +
      labs(title = "NCEI-NOAA Ocean Heat Content Changes",
           x = "Year",
           y = "Heat Content (ZJ)")
#+END_SRC

So, the overall average increase in heat content over the full data set is
#+BEGIN_SRC R
  lm(ohc ~ year, data=noaa_ohc)
#+END_SRC
that is to say, since 1958, the heat content of the oceans has
increased on average by \unit{5.16}{\zettad\joule} every year.

Restricting ourselves to the changes since 1995, we get
#+BEGIN_SRC R
  lm(ohc ~ year, data=filter(noaa_ohc, year > 1995))
#+END_SRC
i.e., some \unit{8.66}{\zettad\joule} every year.

Bearing in mind that the area occupied by the world's oceans covers
\unit{361.84}{\terad\metre\squared} and that one year is made up by
\unit{31.558}{\megad\second} we calculate the equivalent average
surface energy flux as

| Time interval | Yearly Heat Increase (\zetta\joule) | Surface Flux (\watt\per\metre\squared) |
|---------------+-------------------------------------+----------------------------------------|
|    1958--2020 |                                5.16 |                                  0.452 |
|    1995--2020 |                                8.66 |                                  0.758 |

In other words, an additional absorption of
\unit{0.758}{\watt\per\metre\squared} would account for the increase
in ocean heat content observed over the last 25 years.
...
profile with the CO_{2} absorption bands

#+BEGIN_SRC R :file output/bb_radiation_co2.pdf
...
#+CAPTION: Temperature dependence of energy available for absorption
#+ATTR_LaTeX: label tbl:black_body
#+TBLNAME: energy665
...
#+BEGIN_SRC R :var energy665=energy665 :file output/energy665.pdf
...
function of CO_{2} concentrations with finer granularity.
...
And so we get the following relationship between CO_{2} concentration
and absorbed energy by the entire atmospheric column
#+BEGIN_SRC R :file output/abs_voigt_ppm.pdf
  ppm_abs %>%
      ggplot() +
...
#+RESULTS[305f167f733ee27789314722d8bb6fb3ec1a6dde]:
#+begin_example
...
multiplying the CO_{2} dependent AbsV value with the amount of energy
...
equilibrium temperatures, given CO_{2} concentrations.

For most of the Holocene, we had 280 ppm of CO_{2} with an average
...
The present 420 ppm concentration of CO_{2} corresponds to
...
(black) body emits less energy in the CO_{2} absorption band:
