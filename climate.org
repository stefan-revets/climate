...
\unit{3700}{\centi\reciprocal\metre} are already saturated by the
H_{2}O absorption).

Taking a closer look around the \unit{5000}{\centi\reciprocal\metre}
region shows that a small amount of absorption can take place here:
...
\centi\reciprocal\metre. Looking more closely at the fortran code
reveals that the absorption coefficient is standardised to the number
of CO_{2} molecules per \metre\cubed, i.e., the mole fraction,
temperature and pressure are used to calculate the number of
molecules: the absorption is then multiplied with this ``density''.
...
This table also shows that the bands around 3625 and
\unit{3727}{\centi\reciprocal\metre}, which are entirely subsumed in
one of the H_{2}O bands, account for barely 2.5% of the overall absorption.

Let's explore the effects of changes in pressure and temperature on
the integrated absorption, using the strongest peak (around
\unit{2361}{\centi\reciprocal\metre}) as reference. The reference
value is calculated with the following settings
...
Perusal of the equations above hint that concentration differences in
CO_{2} will have very little effect on the individual absorption,
particularly at the ppm concentrations we are dealing with here.

First, the dependence between absorbance and pressure
#+TBLNAME: Press_Abs
| pressure |  IAbs_V |  IAbs_f |  IAbs_F |
|----------+---------+---------+---------|
|      1.0 | 1.07069 | 1.07141 | 1.07130 |
|      0.5 | 0.53553 | 0.53571 | 0.53562 |
|      0.1 | 0.10690 | 0.10690 | 0.10702 |
|     0.05 | 0.05282 | 0.05282 | 0.05321 |
|     0.01 | 0.00891 | 0.00891 | 0.01045 |

#+BEGIN_SRC R :var Press_Abs=Press_Abs :file output/hitran_PA.pdf
  Press_Abs %>%
      mutate(Abs_F = IAbs_F / (pressure * Press_Abs$IAbs_F[1])) %>%
      ggplot() +
      geom_point(aes(pressure * p0/1000, Abs_F)) +
...
again, removing the density effect then yields
#+BEGIN_SRC R :var Temp_Abs=Temp_Abs :file output/hitran_TA.pdf
  Temp_Abs %>%
      mutate(Abs_F = IAbs_F * Temperature /
                 (Temp_Abs$IAbs_F[1]* Temp_Abs$Temperature[1])) %>%
      ggplot() +
      geom_point(aes(Temperature, Abs_F)) +
      labs(title = "Relative Absorbance in function of Temperature",
           x = "Temperature (K)",
           y = "Relative Absorbance")
#+END_SRC

These last two graphs show that the relative effects of pressure and
...
\unit{2360}{\centi\reciprocal\metre} wave number. From the atmospheric
profile for a mid-latitude summer, we then obtain

#+TBLNAME: abs_mid_summer
| h (\kilo\metre) | P (\kilo\pascal) | T (\kelvin) | H_{2}O (ppm) | Absorption |
|-----------------+------------------+-------------+--------------+------------|
|             0.0 |            101.3 |       294.2 |        18800 |    1.05665 |
|             1.0 |            90.11 |       289.7 |        13800 |    0.96523 |
|             2.0 |            80.02 |       285.2 |         9680 |    0.87213 |
|             3.0 |            70.90 |       279.2 |         5980 |    0.78962 |
|             4.0 |            62.66 |       273.2 |         3810 |    0.71370 |
|             5.0 |            55.22 |       267.2 |         2230 |    0.63389 |
|             6.0 |            48.54 |       261.2 |         1510 |    0.57757 |
|             7.0 |            42.53 |       254.7 |         1020 |    0.51862 |
|             8.0 |            37.13 |       248.2 |          646 |    0.46469 |
|             9.0 |            32.31 |       241.7 |          413 |    0.41549 |
|            10.0 |            28.01 |       235.3 |          247 |    0.36473 |
|            11.0 |            24.19 |       228.8 |         95.6 |    0.32891 |
|            12.0 |            20.80 |       222.3 |         29.4 |    0.29099 |
|            13.0 |            17.80 |       215.8 |          8.0 |    0.25655 |
|            14.0 |            15.21 |       215.7 |          5.0 |    0.21918 |
|            15.0 |            12.99 |       215.7 |          3.4 |    0.18485 |
|            20.0 |             5.93 |       219.2 |          3.3 |    0.08360 |
|            25.0 |             2.76 |       225.1 |          4.2 |    0.04389 |
|            30.0 |             1.32 |       233.7 |          4.7 |    0.01999 |


#+BEGIN_SRC R :var abs_profile=abs_mid_summer :file output/abs_profile.pdf
  names(abs_profile) <- c("Height", "Pressure", "Temperature",
                          "Water", "Absorption")

  xCO2 <- 420e-6
  abs_profile %<>%
      mutate(CO2 = Pressure *1e3 * N_A * xCO2 / (R_gas * Temperature),
             Mol_absorption = Absorption / CO2)

  ggplot(abs_profile) +
      geom_line(aes(Height, Mol_absorption))
#+END_SRC

which shows very nicely that the absorption does indeed decrease less
rapidly than what the pressure profile alone would dictate.
