...
latitude, or are these essentially the same? Let's offset the three
zones from each other in the plot by 0.5, so that we can see the
individual lines more clearly.
...
The same calculation, but this time for the Earth, with an average
temperature of \unit{288}{\kelvin}
#+BEGIN_SRC R :file output/earth_spectrum.pdf
  T_earth <- 288

  earth_energy <- data.frame(lambda = seq(2000, 50000, 200)*1e-9) %>%
      mutate(E1 = 8 * pi * h_planck * c_light / lambda^5,
             E2 = h_planck * c_light / (lambda * k_boltzmann * T_earth),
             Energy = E1 / (exp(E2) - 1))
...
available from the hitran site and allows for easy experimentation.
The following variables can be changed before carrying out the
calculations
