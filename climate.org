...
pressure, and temperature on the integrated absorption, using the
strongest peak (around \unit{2361}{\centi\reciprocal\metre}) as
reference. The reference value is calculated with the following
settings
...
Of course, we have to undo the density effect first

That also applies to any effect of pressure on absorbance
...
again, the density effect has to be removed here as well
#+BEGIN_SRC R :var Temp_Abs=Temp_Abs :file output/hitran_TA.pdf
  ggplot(Temp_Abs) +
      geom_point(aes(Temperature, IAbs_F / Temperature)) +
      xlim(200, 300) +
...
Hence, the interplay between temperature and pressure in the
atmospheric column needs to be brought into consideration, as the
supra-linear temperature absorbance will offset the linear pressure
absorbance effect. This can be shown by using the values of
temperature, pressure and water content of the earlier mentioned model
atmosphere profiles, and calculating the integrated absorbance at 420
ppm CO_{2}, again \unit{150}{\centi\reciprocal\metre} around the
\unit{2361}{\centi\reciprocal\metre} wave number. From the atmospheric
...
|            20.0 |             5.93 |       217.9 |          3.2 |    0.08360 |
|            25.0 |             3.21 |       223.9 |          4.0 |    0.04389 |
|            30.0 |             1.53 |       231.6 |          4.6 |    0.01999 |
