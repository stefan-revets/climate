...
\unit{2360}{\centi\reciprocal\metre} wave number.

#+BEGIN_SRC sh :session *bash* :results none :eval never
  cd hitran2020

  awk 'NR > 2 && $1 < 46 {print "SgMinR  2275.0\nSgMaxR  2425.0\ndSg     0.01\nTotMax  0.1e-28\nxCO2    420.e-6\nMixFull 0\nMixSDV  1\nTemp    "$3 "\nPtot    "$2/1013.25 "\nxH2O    "$4}' ../data/grl54358_atmosphere_midlat_summer.dat > lm_input.new
  ...
  for i in `awk 'BEGIN { for( i=1; i<=46; i++ ) print i }'`; do
      ...
      ./lm_calc
      awk -v h=$my_h '{AbsV += $2;AbsL += $3}END{print h, AbsV/100, AbsL/100}' ABSCO_HIT2020.dat >> ../cache/atm_profile_results.dat 
  done
...
which is consistent with the pressure-temperature dependence examined above.
