...
#+BEGIN_SRC R :results none
  ...
  h_plank <- 6.62606957e-34       # J s, Plank constant
...
much energey is captured by the greenhouse gases, i.e., water vapour,
...
remarkably well as that of a black body. The same principle can be
applied to the Earth as well.

Black body radiation was first analysed and described by Plank as
\begin{equation}
E(\nu, T) = \frac{2 \pi \nu^2}{c^2} \frac{h \nu}{e^{h \nu / k T} - 1}
\end{equation}
...
#+BEGIN_SRC R :file output/solar_spectrum.pdf
  ...
  sun_energy <- data.frame(lambda = seq(100, 2500, 10)*1e-9) %>%
      mutate(nu = c_light / lambda,
             Energy = 2 * pi * nu^3 * h_plank / (c_light^2 *
             (exp(h_plank * nu/(k_boltzmann * T_sun)) - 1)))
