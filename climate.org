...
around an absorption peak at \unit{4.24}{\micro\metre} (or, in the cgs
...
one of the H_{2}O bands, account for barely 2.5% of the overall
absorption, while the bands around
\unit{5000}{\centi\reciprocal\metre} account for even less than
that. This shows us that we only need to consider the bands around 665
and \unit{2340}{\centi\reciprocal\metre}.
...
\unit{2340}{\centi\reciprocal\metre}) as reference. The reference
...
#+BEGIN_SRC R :var Press_Abs=Press_Abs :file output/hitran_PA.pdf
  Press_Abs %>%
      mutate(Abs_V = IAbs_V / (pressure * Press_Abs$IAbs_V[1])) %>%
      ggplot() +
      geom_point(aes(pressure * p0/1000, Abs_V)) +
...
#+BEGIN_SRC R :var Temp_Abs=Temp_Abs :file output/hitran_TA.pdf
  Temp_Abs %>%
      mutate(Abs_V = IAbs_V * Temperature /
                 (Temp_Abs$IAbs_V[1]* Temp_Abs$Temperature[1])) %>%
      ggplot() +
      geom_point(aes(Temperature, Abs_V)) +
...
#+BEGIN_SRC R :file output/abs_profile.pdf
  abs_profile <-
      read.table("data/atm_profile_abs.dat",
                 col.names = c("Height", "Pressure", "Temperature", "P_atm",
                               "Water", "Abs_Voigt", "Abs_fo", "Abs_fl"),
                 skip = 1)

  ggplot(abs_profile) +
      geom_line(aes(Height, Abs_Voigt)) +
      labs(title = "Atmospheric Absorption Profile",
           x = "Height (km)",
           y = "Absorption")
#+END_SRC

As the density of CO_{2} is the main driver in this absorption
profile, let's see how the individual molecule absorption changes with
...
#+BEGIN_SRC R :file output/abs_profile_molecule.pdf
  ...
  ggplot(abs_profile) +
      geom_line(aes(Height, Voigt*1e23)) +
      labs(title = "Molecule-specific  Absorption Profile",
           x = "Height (km)",
           y = expression(paste("Absorption ", (10^{-23}))))
#+END_SRC
