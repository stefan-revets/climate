...
#+BEGIN_SRC R :file output/delavega_co2.pdf
  ...
  ggplot(delavega_boron) +
      geom_point(aes(Age, Boron, colour = origin)) +
      geom_line(aes(Age, Boron, colour = origin)) +
      labs(title = "pCO2 from Boron in Pliocene Foraminifera",
           x = "Time (kY)",
           y = "pCO2") +
...
species \textit{Globigerinoides ruber} and \textit{Globigerinoides
sacculifer} at similar (same?) sample depths routinely exceed the
variations recorded over the time interval in one and the same
species. This is fundamentally problematical.
...
#+BEGIN_SRC R :file output/delavega_supp.pdf
  ...
  ggplot(delavega_sm) +
      ...
      labs(title = "Boron offsets in Pliocene Foraminifera",
           x = "Time (kY)",
...
#+BEGIN_SRC R :file output/delavega_supp_co2.pdf
  ggplot(delavega_sm) +
      ...
      labs(title = "CO2 from Boron in Pliocene Foraminifera",
           x = "Time (kY)",
           y = "pCO2") +
      theme(legend.position = "bottom")
#+END_SRC
...
spreadsheet data does contain significantly higher values.

It seems best, once again, to go to the original publications and try
to obtain the data from there.
...
And, after calculations (and assumptions!), estimates of CO_{2}
