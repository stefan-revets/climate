...
| isotope         | proportion |
|-----------------+------------|
| \isotope{10}{B} | 0.19217    |
| \isotope{11}{B} | 0.80783    |
...
#+BEGIN_SRC R :file output/boron_pH.pdf
  ...
  boron_pH <- data.frame(pH = seq(6, 11, 0.1), T = 283) %>%
      ...
      mutate(o_boron = exp(lnKB(T, S = 35)) * boron_total / (10^(-pH) + exp(lnKB(T, S = 35)))) %>%
      mutate(Temperature = as.factor(T))

  ggplot(boron_pH) +
      geom_line(aes(pH, o_boron*1e6, colour = Temperature)) +
      labs(title = "Seawater [o-Borate] pH dependence at different Temperatures",
           x = "pH",
...
A simple regression analysis gives us
#+BEGIN_SRC R :var kakihana=kakihana
  lm(K_isoB ~ T, data=kakihana)
#+END_SRC

#+RESULTS[f0c4a7d7c070672db8a467a5117812f9b20b48c4]:
: 
: Call:
: lm(formula = K_isoB ~ T, data = kakihana)
: 
: Coefficients:
: (Intercept)            T  
:   1.0338678   -0.0000485
...
\begin{equation}
K_{\isotope{10}{B}-\isotope{11}{B}} = 1.0339 - 0.0000485 \, T
\end{equation}
...
Now to calculate and predict the \delta\isotope{11}{B} of seawater as a
function of pH, and temperature.

After a bit (!) of algebra, I have come up with the equation
\begin{equation}
\frac{[\isotope{11}{B}\mathrm{(OH)}^-_4]}{[\isotope{10}{B}\mathrm{(OH)}^-_4]} = \mathrm{K}_{\isotope{11-10}{B}} \frac{[\isotope{11}{B}]}{[\isotope{10}{B}]} \frac{[\mathrm{H}^+] + \mathrm{K}_B \mathrm{K}^{-r_{11}}_{\isotope{11-10}{B}}}{[\mathrm{H}^+] + \mathrm{K}_B \mathrm{K}^{r_{10}}_{\isotope{11-10}{B}}}
\end{equation}
in which $r_{10}, r_{11}$ stand for the proportion of the boron
isotopes to the boron total in seawater.
...
#  K11_10 <- 1.0272
  ...
  B10_sea <- 0.19217
  B11_sea <- 0.80783
  B11_10_sea<- B11_sea / B10_sea

  B10_951 <- 0.19827
  B11_951 <- 0.80173
  B11_10_951 <- B11_951 / B10_951

  K_r11 <- K11_10^(-B11_sea)
  K_r10 <- K11_10^B10_sea

  boron_delta <- data.frame(pH = seq(6, 11, 0.1), T = 283) %>%
      rbind(data.frame(pH = seq(6, 11, 0.1), T = 288)) %>%
      rbind(data.frame(pH = seq(6, 11, 0.1), T = 293)) %>%
      mutate(K_B = exp(lnKB(T, S = 35))) %>%
      mutate(b_ratio = K11_10 * B11_10_sea * (10^(-pH) + K_B * K_r11) /
                 (10^(-pH) + K_B * K_r10)) %>%
      mutate(delta_B = (b_ratio / B11_10_951 - 1) * 1000,
             Temperature = as.factor(T))

  ggplot(boron_delta) +
      geom_line(aes(pH, delta_B, colour = Temperature)) +
      labs(title = "Borate fractionation in function of pH and Temperature",
           x = "pH",
           y = expression(paste(delta ^{11}, "B"))) +
      theme(legend.position = "bottom")
#+END_SRC
