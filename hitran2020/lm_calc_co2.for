      program Example_Main_LM_CO2
C-------------------------------------------------
C       The code is mostly based on
C       Lamouroux, et al., JQSRT. 151 (2015), 88-96
C       with some updates described in 
C       Hashemi,et al., JQSRT. 256, (2020), 107283. 
C       This version of the code is distributed with the HITRAN2020 edition
C
C       Users, please cite the following references:
C	J. Lamouroux, L. Realia, X. Thomas, et al., JQSRT. 151 (2015), 88-96
C       Hashemi,et al., JQSRT. 256, (2020), 107283. 
C
C	INPUT VARIABLES (supplied by the user)
C --------------
C	sgmin, sgmax, dsg [cm-1] : Min and Max wavenumbers and step of the calculation
C	sTotMax [cm-1/(molecule.cm-2) at 296K]: Total band intensity cut-off
C	xCO2 [no unit] : CO2 molefraction
C	xH2O [no unit] : H2O molefraction
C	Temp [K] : Temperature of the calculation
C	Ptot [atm] : Total pressure  
C  
C     MixFull = Switch to full diagonalization line-mixing
C	MixSDV = Switch to a Speed-dependent Voigt profile
C	Note that the full diagonalization line-mixing is not possible when
C       a Speed-dependent Voigt profile is used
C
C
C	RESULTS 
C --------------
...
C
C     The USER will probably HAVE TO CHANGE the access path to 
...
C Total band intensity cut-off (user supplied)
	sTotMax=0.1d-27
     ...
     $   ,action="write", status="unknown")
        open (unit=203,file="./output/00_YT_Zero_.dat"
     $   ,action="write", status="unknown")
C Call routine to select band according to WN range and cut-off
c
      call DetBand(SgMinR,SgMaxR,stotmax)
c
...
C 
	if(MixSDV) then
C SHOULD NOT BE MODIFY BY THE USER: Calculations using
C a Speed-dependent Voigt profile can NOT be done with full relaxation matrix
	 MixFull=.FALSE.
...
C print results in File
      open(unit=55,file='ABSCO_HIT2020.dat',status='unknown')
      do isig=1,Nsig
	sig=sgminR+Dfloat(isig-1)*Dsg
      write(55,123) sig,AbsV(isig), AbsY(isig), AbsW(isig)
      enddo
      close(55)
      close(203)
      close(202)
...
C
C******************************************************************************
      Subroutine DetBand(SgMinR,SgMaxR,stotmax)
C******************************************************************************
...
C Modified by J. Lamouroux, last change Feb. 2015
C*********************************************************************
C
      ...
      integer*4 li,lf,lir,lfr,iskip,iband0
...
C Open File Giving Information on Bands, Read, and Select
      nBand=0
      Open(Unit=iFile,File='data_new/BandInfo.dat',
     &Status='Old')
...
C
          Isot(nBand)=IsotR
C    In HITRAN the isotopologue "0" is the 10th one (for TIPS)
	    if(IsotR.eq.0)Isot(nBand)=10
          li(nBand)=liR
          lf(nBand)=lfR
...
C Accessed Files
C --------------
C For all the couple (li,lf) where (lf>=li, and li<=8) the  file
C	"Data_new\WTfitXY.dat" with X=li, Y=lf 
...
C
C     F. Niro, last change 15 Jan 2005
C     J. Lamouroux, last change Dec. 2013
C*********************************************************************
...
c
          ...
          cr=cr1//cr2
          i=1
          open(unit=10,file='data_new/WTfit'//cr//'.dat', 
     &         Status='Old')
...
C     HWVT0AIR : Air-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm) for the VOIGT profile
C     HWSDVT0AIR : Air-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm) for the SPEED-DEPENDENT VOIGT profile
C	rHWT0AIR: Air-broadened Half-Widths Ratios (at 296 K) Gam2/HWSDVT0AIR
C     BHWAIR : Temperature Dependence Coefficients of HWT0AIR
C	HWVT0SELF:Self-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm) for the VOIGT profile
C     HWSDVT0SELF : Self-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm) for the SPEED-DEPENDENT VOIGT profile
C	rHWT0SELF: SELF-broadened Half-Widths Ratios (at 296 K) Gam2/HWSDVT0SELF
C     BHWSELF : Temperature Dependence Coefficients of HWT0SELF
C     HWVT0H2O : H2O-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm) for the VOIGT profile
C     HWSDVT0H2O : H2O-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm) for the SPEED-DEPENDENT VOIGT profile
C	rHWT0H2O: H2O-broadened Half-Widths Ratios (at 296 K) Gam2/HWSDVT0H2O
...
C     convention used in the HITRAN-2012 Data base).
C     The USER will probably HAVE TO CHANGE the access Paths to 
C     these files according to his computer+directory system.
C     
C     Called Routines: None
...
C     
C     F. Niro, last change 15 Jan 2005
C	J. Lamouroux, last change last change Feb. 2015
C*********************************************************************
C     
	...
	logical MixSDV
      ...
      real*8 HWVT0AIR,HWSDVT0AIR,rHWT0AIR,BHWAIR
      real*8 HWVT0SELF,HWSDVT0SELF,rHWT0SELF,BHWSELF
      real*8 HWVT0H2O,HWSDVT0H2O,rHWT0H2O,BHWH2O
	real*8 shft0
	real*8 Sig,Dipo0,E,PopuT0,S
      real*8 DipoT,pop,Intens
      Character*1 TpLine
	Character*14 BandFile
C     
C     Characteristics of the Bands
...
C     Data of the Lines at Ref Temperature/Pressure
      ...
      Common/GamVT0AIR/HWVT0AIR(nLmx,nBmx) 
      Common/GamSDVT0AIR/HWSDVT0AIR(nLmx,nBmx),rHWT0AIR(nLmx,nBmx) 
      Common/DTGAMAIR/BHWAIR(nLmx,nBmx)
      Common/GamVT0CO2/HWVT0SELF(nLmx,nBmx) 
      Common/GamSDVT0CO2/HWSDVT0SELF(nLmx,nBmx),rHWT0SELF(nLmx,nBmx) 
      Common/DTGAMCO2/BHWSELF(nLmx,nBmx)
      Common/GamVT0H2O/HWVT0H2O(nLmx,nBmx) 
      Common/GamSDVT0H2O/HWSDVT0H2O(nLmx,nBmx),rHWT0H2O(nLmx,nBmx) 
      Common/DTGAMH2O/BHWH2O(nLmx,nBmx)
      Common/SHIFT0/shft0(nLmx,nBmx) 
...
C     
         nLineR=1
         Open(Unit=iFile,
     &File='data_new/'//Bandfile(iband)//'.dat',Status='Old')
 2     Read(iFile,1001,End=100)Isot(iBand),Sig(nLineR,iBand),Intens
     ,,HWVT0AIR(nLineR,iBand),HWSDVT0AIR(nLineR,iBand)
     ,,rHWT0AIR(nLineR,iBand)
     ,,HWVT0SELF(nLineR,iBand),HWSDVT0SELF(nLineR,iBand)
     ,,rHWT0SELF(nLineR,iBand)
     ,,E(nLineR,iBand),BHWAIR(nLineR,iBand),BHWSELF(nLineR,iBand)
     ,,shft0(nLineR,iBand),TpLine,Ji(nLineR,iBand)
     ,,HWVT0H2O(nLineR,iBand),HWSDVT0H2O(nLineR,iBand)
     ,,rHWT0H2O(nLineR,iBand),BHWH2O(nLineR,iBand)
...
C    In HITRAN the isotopologue "0" is the 10th one (for TIPS)
	if(Isot(iBand).eq.0)Isot(iBand)=10

1001  FORMAT(2x,I1,F12.6,E10.3,10x,2(2f5.4,f4.2),f10.4,2f4.2,
     +f8.6,50x,A1,I3,21x,(2f5.4,f4.2),f5.3,2d16.8)        
...
C*********************************************************************
C*********************************************************************
      Subroutine CompAbs(Temp,Ptot,xCO2,xH2O,SigMin,SigMax,DSig,
     &                   AbsV,AbsY,AbsW,MixSDV,MixFull,Nsig)
C*********************************************************************
...
C     
C     F. Niro, last change 15 Jan 2005
C     J. Lamouroux, last change Feb. 2015
C     R. Hashemi, last change June 2021
C*********************************************************************
C     
      Implicit None
      include 'parameters.inc'
      ...
      integer*4 nSig,iSig,iBand,iLine,ji,jf, sm
      real*8 Temp,Ptot,xCO2,xH2O,SigMin,SigMax,DSig
      real*8 Sig,Dipo0,SHFT,SHFT0,PopuT,YT,DipoT
      real*8 HWT,HWSDV2T
      real*8 GamDmx,ds0,ds2,dsigc
      real*8 order2R,order2I
      real*8 SSR,SSI,AlphR,AlphI,Dens,PopuDipo
      real*8 SigMoy,GamD,Cte,Cte1,Fact, GamDFull,CteFull
      real*8 SigC,SumV(nSigmx),SumY(nSigmx),pptl
      real*8 XX,YY,WR,WI
      real*8 StotB
      real*8 sum_Dipo0, sum_YT
      real*8 SqrTm,rdmult
      real*8 sq_ln2,sq_ln2pi,u_pi,u_sqln2pi
C     Characteristic of the Bands
C	Character*14 BandFile
      Character*3 tpline
      Character*3 smline
...
C     Data of the "Real" Lines
      Common/LineSg/Sig(nLmx,nBmx)
      Common/DipoRigid/Dipo0(nLmx,nBmx)
      Common/GamT/HWT(nLmx),HWSDV2T(nLmx)
      Common/SHIFT/shft(nLmx)
      Common/PopuT/PopuT(nLmx)
      Common/DipoTcm/DipoT(nLmx,nBmx)
      Common/Jiln/Ji(nLmx,nBmx)
      Common/Jfln/Jf(nLmx,nBmx)
      Common/smln/sm(nLmx,nBmx)
...
C
      Do 1 iBand=1,nBand
         sum_Dipo0 = 0
         sum_YT = 0
         print*, 'Band treated',iBand,'/',nBand ,'/',
     &   Bandfile(iBand)  , '# lines:',nLines(iBand)

C     Convert Current Band Data to Considered Temperature and Pressure
         Call ConvTP(iBand,Isot(iBand),nLines(iBand),xH2O,xCO2,Temp,
     &               Ptot,SigMoy,GamDmx,MixFull,MixSDV)


C        Print the Rosenkranz line-mixing parameters for each band
        open (unit=23,file="./output/YT_"//Bandfile(iBand)//".dat"
     $   ,action="write", status="REPLACE")
         ...
         Do 4 iLine=1,nLmx
            ...
            if(Jf(iLine,iBand).EQ. Ji(iLine,iBand)-1)then
            TpLine = 'P'      !P line
            end if
	    if(Jf(iLine,iBand).EQ. Ji(iLine,iBand))then
               TpLine ='Q' ! Q line
            end if
            if (Jf(iLine,iBand).EQ. Ji(iLine,iBand)+1)then
	       TpLine = 'R'
	  ...
	  if (iLine .LE. nLines(iBand)) then
	   if (abs(YT(iLine)) .gt. 0.08) then
	        YT(iLine)= 0.0
	        end if
           if (abs(YT(iLine)) .LE. -0.08) then
               YT(iLine)= 0.0
	        end if
          if (YT(iLine) .EQ. 0) then
       write(203,301) Bandfile(iBand), TpLine, Jf(iLine,iBand),
     & Ji(iLine,iBand)
301       FORMAT(2A, 2I12)
       else
           write(202,302) Bandfile(iBand),
     &  TpLine, Jf(iLine,iBand), Ji(iLine,iBand),YT(iLine)
302       FORMAT(2A, 2I12, F10.6)
     
       end if
         write (23,617) TpLine, smLine, TpLine, Ji(iLine,iBand),
     &     Jf(iLine,iBand), YT(iLine)
617      FORMAT(3A, 2I12,F10.6)
         end if
 4    Continue
         close(23)
...
C...  Modelisation of the GamD/Gam0 dependence of the cut-off for the 
C...  0th and 2nd order approximation of the Speed-dependent Lorentz profile
C...  These functions have been obtained for Gam2/Gam0=0.5 
	ds0=(70.67d0+104.1d0*0.21d0*GamDmx**6.4)/(1.d0+0.21d0*GamDmx**5.4)
	ds2=(34.97d0+105.d0*9.d0*GamDmx**3.1)/(1.d0+9.d0*GamDmx**2.1)
...
C     Compute the Doppler Width for Full Line Mixing using correction suggested by 
C     Richard Larsson, MPI Gottingen (2020).
C     This correction fixes previous bug regarding frequency sorting
C------------------------------------------------------------------------------
               GamDFull=CtGamD*(AlphR(iLine)+SigMoy)*SqrTm
               CteFull=sq_ln2/GamDFull
C--------------------------------------------------------------------------------
               PopuDipo=PopuT(iLine)*(DipoT(iLine,iBand)**2)

			 dsigc=(SigC-Sig(iLine,iBand)-SHFT(iLine)*Ptot)
			 
C...  Speed-dependence
			 IF(MixSDV) then

C...  Approximate the Speed-dependent Lorentz by the 0th order
                IF(dabs(dsigc)/HWT(iLine).gt.ds0)THEN

                 AbsV(iSig)=AbsV(iSig)+PopuDipo*(u_sqln2pi)*
     &      (u_pi)*(HWT(iLine))/(HWT(iLine)**2+dsigc**2)

                 AbsY(iSig)=AbsY(iSig)+PopuDipo*(u_sqln2pi)*
     &      (u_pi)*(HWT(iLine)+YT(iLine)*dsigc)/(HWT(iLine)**2+dsigc**2)

C...  Approximate the Speed-dependent Lorentz by the 2nd order
C...  Note that the 1st order is zero.
		      ELSEIF(dabs(dsigc)/HWT(iLine).gt.ds2 .and.
     +		        dabs(dsigc)/HWT(iLine).le.ds0 )THEN

			   Order2R=HWT(iLine)/(HWT(iLine)**2+dsigc**2)+
     + 3.d0*HWSDV2T(iLine)**2*(HWT(iLine)**3-3.d0*HWT(iLine)*dsigc**2)/
     + (2.d0*pi*(HWT(iLine)**2+dsigc**2)**3)

			   Order2I=dsigc/(HWT(iLine)**2+dsigc**2)+
     + 3.d0*HWSDV2T(iLine)**2*(3.d0*HWT(iLine)**2*dsigc-dsigc**3)/
     + (2.d0*pi*(HWT(iLine)**2+dsigc**2)**3)

                 AbsV(iSig)=AbsV(iSig)+PopuDipo*(u_sqln2pi)*
     &        (u_pi)*Order2R

                 AbsY(iSig)=AbsY(iSig)+PopuDipo*(u_sqln2pi)*(u_pi)
     &        *(Order2R+YT(iLine)*order2I)


C	Calculate Speed-dependent Voigt profile
			  ELSE
                   SHFT0=SHFT(iLine)*Ptot
			 CALL qSDV(Sig(iLine,iBand),GamD,HWT(iLine),
     &			 HWSDV2T(iLine),SHFT0,0.d0,SigC,WR,WI)

C     SDVoigt Absorption Coefficient
                   AbsV(iSig)=AbsV(iSig)+PopuDipo*WR*(u_sqln2pi)
     &
...
c
     ...
     &            (rdmult*GamDFull))THEN


C     Complex Probability Function for the "Equivalent" Q-Lines based
C     on CteFull and GamDFull suggested by Richard Larsson

                  XX=(AlphR(iLine)+SigMoy-SigC)*CteFull
                  YY=AlphI(iLine)*CteFull
                  Call CPF(XX,YY,WR,WI)
C     Absorption Coefficient due to W
                  AbsW(iSig)=AbsW(iSig)+
     &                 (SSR(iLine)*WR-SSI(iLine)*WI)/GamDFull
...
C*********************************************************************
C*********************************************************************
      Subroutine ConvTP(iBand,IsotC,nLineC,xH2O,xCO2,Temp,Ptot
     ,              ,SigMoy,GamD_gam0_mx,MixFull,MixSDV)
C*********************************************************************
C     
...
C     Input/Output Parameters of Routine (Arguments or Common)
C     ---------------------------------
C     MixFull = Switch to full diagonalization line-mixing (Input).
C	MixSDV = Switch to a Speed-dependent Voigt profile (Input).
C
...
C     Temp   : Temperature in Kelvin (Input).
C     Ptot   : Total Pressure in Atmosphere (Input).
C     xH2O   : H2O Mole fraction (Input).
...
C     --------------
C     
C     Called Routines: "QTCO2" (Partition Function of CO2, TIPS 2011)
...
C     
      ...
      integer*4 iLine,iLineP,Isot,Ji,Jf,lii,lff
      real*8 SigMoy,Temp,Ptot,xH2O,xCO2,GamD_gam0_mx
      real*8 HWVT0AIR,HWSDVT0AIR,rHWT0AIR,BHWAIR
      real*8 HWVT0SELF,HWSDVT0SELF,rHWT0SELF,BHWSELF
      real*8 HWVT0H2O,HWSDVT0H2O,rHWT0H2O,BHWH2O
	real*8 HWT,HWSDV2T
	real*8 GamD,SqrTm
	real*8 Sig,Dipo0,E,S
 	real*8 gsi0,gsit,QT0_CO2,QTT_CO2
      real*8 PopuT0,PopuT,YT,DipoT,SHFT0,SHFT
...
C     Compute useful quantities out of the loop
      RatioT=T0/Temp
c$$	Updated partitions functions, JL
	CALL QT_CO2(T0,IsotC,gsi0,QT0_CO2) 
	CALL QT_CO2(Temp,IsotC,gsit,QTT_CO2) 
      RatioPart=QT0_CO2/QTT_CO2	
C
	GamD_gam0_mx=-1.d0
      SqrTm=DSQRT(Temp/aMass(IsotC))
C     
C     Treat all Lines for Temperature    
     ...
     &        *dExp(-Ct*E(iLine,iBand)*(1.d0/Temp-1.d0/T0))
	   IF(MixSDV) then	
          HWT(iLine)=((1-xH2O-xCO2)*(HWSDVT0AIR(iLine,iBand)			
     &        *(RatioT**BHWAIR(iLine,iBand))))
     +   +(xH2O*(HWSDVT0H2O(iLine,iBand)			
     +        *(RatioT**BHWH2O(iLine,iBand))))
     +   +(xCO2*(HWSDVT0SELF(iLine,iBand)
     +        *(RatioT**BHWSELF(iLine,iBand))))
          HWSDV2T(iLine)=((1-xH2O-xCO2)			
     +        *(HWSDVT0AIR(iLine,iBand)*rHWT0AIR(iLine,iBand)
     &        *(RatioT**BHWAIR(iLine,iBand))))
     +   +(xH2O*(HWSDVT0H2O(iLine,iBand)*rHWT0H2O(iLine,iBand)			
     +        *(RatioT**BHWH2O(iLine,iBand))))
     +   +(xCO2*(HWSDVT0SELF(iLine,iBand)*rHWT0SELF(iLine,iBand)
     +        *(RatioT**BHWSELF(iLine,iBand))))
	   ELSE
          HWT(iLine)=((1-xH2O-xCO2)*(HWVT0AIR(iLine,iBand)			
     &        *(RatioT**BHWAIR(iLine,iBand))))
     +   +(xH2O*(HWVT0H2O(iLine,iBand)			
     +        *(RatioT**BHWH2O(iLine,iBand))))
     +   +(xCO2*(HWVT0SELF(iLine,iBand)
     +        *(RatioT**BHWSELF(iLine,iBand))))
	   ENDIF
	   	
         shft(iLine)=shft0(iLine,iBand)
...
C     Treat all Lines for Pressure   
      Do iLine=1,nLineC
         ...
         HWSDV2T(iLine)=Ptot*HWSDV2T(iLine)
         YT(iLine)=Ptot*YT(iLine)
C	Determine the GamD/Gam0 max for this band
         GamD=CtGamD*Sig(iLine,iBand)*SqrTm
	   GamD_gam0_mx=dmax1(GamD_gam0_mx,GamD/HWT(iLine))
      Enddo
...
C*********************************************************************
      ...
      real*8 Sig,Dipo0,E,HWT,HWSDV2T,PopuT,DipoT,shft
...
C
      do i=1,nraies
         ...
         enddo
      enddo
	if(li(iband).gt.8.or.lf(iband).gt.8.
...
C     Range the ray for decreasing S
      do i=1,nraies
	    ...
	    if(S(j).gt.S(i))then
            ...
            call  Switch(HWSDV2T,j,i,nraies)
            call  Switch(shft,j,i,nraies)
...
C     Correction for asym isotopologues
            if(Isot(iBand).gt.2.AND.Isot(iBand).ne.7.AND.
     &           Isot(iBand).ne.10.AND.
...
c  ***************************************************************************
C  * Partition sums in the line-mixing code are updated in 2020 by Robab Hashemi 
C  * This subroutine calculates the total internal partition sum (TIPS2017) for
C  * CO2 and its isotopologues in the temperature range 70 - 3000 K.
C  * This subroutine calculates the TIPS by 4-point Lagrange interpolation
C  * Taken from the subroutine "BD_TIPS_2017" from R. Gamache et al.
C  * Note that the isotopologue "0" was labelled isotopologue "10" in TIPS
c  ***************************************************************************
      Subroutine QT_CO2   (                       
...
c...      CO2
c...        --       626
      data (QofT( 1,J),J=1,119)/5.36049E+01, 7.58747E+01, 9.81738E+01,
     +1.20608E+02, 1.43391E+02, 1.66808E+02, 1.91167E+02, 2.16767E+02,
     +2.43882E+02, 2.72765E+02, 3.03648E+02, 3.36749E+02, 3.72278E+02,
     +4.10438E+02, 4.51432E+02, 4.95463E+02, 5.42739E+02, 5.93470E+02,
     +6.47873E+02, 7.06170E+02, 7.68592E+02, 8.35378E+02, 9.06772E+02,
     +9.83031E+02, 1.06442E+03, 1.15121E+03, 1.24369E+03, 1.34214E+03,
     +6.36185E+04, 6.65250E+04, 6.95376E+04, 7.26589E+04, 7.58920E+04,
     +7.92396E+04, 8.27049E+04, 8.62909E+04, 9.00006E+04, 9.38371E+04,
     +9.78037E+04, 1.01904E+05, 1.06140E+05, 1.10516E+05, 1.15036E+05,
     +1.19702E+05, 1.24518E+05, 1.29487E+05, 1.34614E+05, 1.39902E+05,
     +1.45354E+05, 1.50974E+05, 1.56766E+05, 1.62733E+05, 1.68880E+05,
     +1.75211E+05, 1.81728E+05, 1.88437E+05, 1.95341E+05, 2.02445E+05,
     +2.09752E+05, 2.17266E+05, 2.24993E+05, 2.32935E+05, 2.41098E+05,
     +2.49485E+05, 2.58102E+05, 2.66951E+05, 2.76039E+05, 2.85369E+05,
     +2.94946E+05/
c...        --       837
      data (QofT( 11,J),J=1,119)/1.40608E+03, 1.99041E+03, 2.57580E+03,
     +3.16578E+03, 3.76693E+03, 4.38765E+03, 5.03671E+03, 5.72241E+03,
     +6.45234E+03, 7.23346E+03, 8.07218E+03, 8.97460E+03, 9.94659E+03,
     +1.09939E+04, 1.21225E+04, 1.33380E+04, 1.46466E+04, 1.60542E+04,
     +1.75674E+04, 1.91925E+04, 2.09364E+04, 2.28061E+04, 2.48088E+04,
     +2.69521E+04, 2.92438E+04, 3.16920E+04, 3.43051E+04, 3.70918E+04,
     +4.00612E+04, 4.32225E+04, 4.65854E+04, 5.01600E+04, 5.39566E+04,
     +5.79857E+04, 6.22585E+04, 6.67863E+04, 7.15808E+04, 7.66542E+04,
     +8.20189E+04, 8.76878E+04, 9.36740E+04, 9.99913E+04, 1.06654E+05,
     +1.13675E+05, 1.21071E+05, 1.28857E+05, 1.37047E+05, 1.45659E+05,
     +1.54708E+05, 1.64212E+05, 1.74188E+05, 1.84654E+05, 1.95627E+05,
     +2.07127E+05, 2.19173E+05, 2.31784E+05, 2.44980E+05, 2.58782E+05,
     +2.73211E+05, 2.88288E+05, 3.04034E+05, 3.20473E+05, 3.37627E+05,
     +3.55519E+05, 3.74172E+05, 3.93612E+05, 4.13862E+05, 4.34948E+05,
     +4.56896E+05, 4.79730E+05, 5.03478E+05, 5.28166E+05, 5.53822E+05,
     +5.80472E+05, 6.08146E+05, 6.36872E+05, 6.66679E+05, 6.97595E+05,
     +7.29651E+05, 7.62876E+05, 7.97301E+05, 8.32956E+05, 8.69872E+05,
     +9.08081E+05, 9.47615E+05, 9.88503E+05, 1.03078E+06, 1.07448E+06,
     +1.11963E+06, 1.16626E+06, 1.21442E+06, 1.26412E+06, 1.31541E+06,
     +1.36832E+06, 1.42287E+06, 1.47912E+06, 1.53708E+06, 1.59679E+06,
     +1.65829E+06, 1.72160E+06, 1.78677E+06, 1.85382E+06, 1.92279E+06,
     +1.99372E+06, 2.06663E+06, 2.14156E+06, 2.21854E+06, 2.29760E+06,
     +2.37878E+06, 2.46212E+06, 2.54763E+06, 2.63535E+06, 2.72532E+06,
     +2.81757E+06, 2.91212E+06, 3.00901E+06, 3.10827E+06, 3.20993E+06,
     +3.31401E+06/
      eps=0.01
c
      gsi = xgj(iso)
